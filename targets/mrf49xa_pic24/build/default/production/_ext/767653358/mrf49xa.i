...
# 5 "C:/Projects/Firmware/MRF49XA/Library/inc/devices/mrf49xa.h" 2
...
void MRF49XA_Reset_Radio()
{
    SPI_Command(0x8200);
    SPI_Command(0xCA81);
    SPI_Command(0x8000 | 0x0008 | 0x0020);
    SPI_Command(0x8200 | 0x0080);
    SPI_Command(0x8000 | 0x0008 | 0x0020 | 0x0040);
    SPI_Command(0xCA81 | 0x0002);
}
...
void MRF49XA_Init()
{
    ...
    SPI_Command(0xC6A3);
    SPI_Command(0x8200);
    SPI_Command((0x9460 | 0x0000 | 0x0000));
    SPI_Command(0x9890);
    SPI_Command(0xC2AC);
    SPI_Command(0x8200 | 0x0020);
...
void MRF49XA_TxPacket(UI08_t *data, UI08_t size)
{
    ...
    SPI_Command(0x8200);
    SPI_Command(0x8000 | 0x0008 | 0x0020 | 0x0080);
    SPI_Command(0x8200 |0x0020);

    LATBbits.LATB5 = 0;

    do { while (!PORTCbits.RC4); } while (0);; SPI_Write16(0xB800 | 0xAA);
    do { while (!PORTCbits.RC4); } while (0);; SPI_Write(0xAA);
    do { while (!PORTCbits.RC4); } while (0);; SPI_Write(0x2D);
    do { while (!PORTCbits.RC4); } while (0);; SPI_Write(0xD4);
    do { while (!PORTCbits.RC4); } while (0);; SPI_Write(size + 1);

    for (i = 0; i < size; i++)
    {
        do { while (!PORTCbits.RC4); } while (0);;
        SPI_Write(data[i]);
        crc = crc ^ data[i];
    }

    do { while (!PORTCbits.RC4); } while (0);; SPI_Write(crc);
    do { while (!PORTCbits.RC4); } while (0);; SPI_Write(0x00);
    do { while (!PORTCbits.RC4); } while (0);; SPI_Write(0x00);
    do { while (!PORTCbits.RC4); } while (0);; SPI_Write(0x00);
    do { while (!PORTCbits.RC4); } while (0);;
