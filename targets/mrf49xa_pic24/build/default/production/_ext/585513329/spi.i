...
void sqiTxByte(UI08_t port, UI08_t byte)
{
    ...
    switch(port)
    {
        case 2:
            ...
            PORTB &= ~(1<<14);
            if (((byte & (1<<4)) != 0)) LATC |= 1<<0; else LATC &= ~(1<<0);
            if (((byte & (1<<5)) != 0)) LATC |= 1<<2; else LATC &= ~(1<<2);
            if (((byte & (1<<6)) != 0)) LATC |= 1<<1; else LATC &= ~(1<<1);
            if (((byte & (1<<7)) != 0)) LATB |= 1<<13; else LATB &= ~(1<<13);
            PORTB |= (1<<14);
            asm("nop");
            PORTB &= ~(1<<14);
            if (((byte & (1<<0)) != 0)) LATC |= 1<<0; else LATC &= ~(1<<0);
            if (((byte & (1<<1)) != 0)) LATC |= 1<<2; else LATC &= ~(1<<2);
            if (((byte & (1<<2)) != 0)) LATC |= 1<<1; else LATC &= ~(1<<1);
            if (((byte & (1<<3)) != 0)) LATB |= 1<<13; else LATB &= ~(1<<13);
            PORTB |= (1<<14);
            asm("nop");
            PORTB &= ~(1<<14);
            break;
...
UI08_t sqiRxByte(UI08_t port)
{
    ...
    switch(port)
    {
        case 2:
            ...
            if (((PORTC & (1<<0)) != 0)) byte |= 1 << 4;
            if (((PORTC & (1<<2)) != 0)) byte |= 1 << 5;
            if (((PORTC & (1<<1)) != 0)) byte |= 1 << 6;
            if (((PORTB & (1<<13)) != 0)) byte |= 1 << 7;

            PORTB &= ~(1<<14);
            PORTB |= (1<<14);

            if (((PORTC & (1<<0)) != 0)) byte |= 1 << 0;
            if (((PORTC & (1<<2)) != 0)) byte |= 1 << 1;
            if (((PORTC & (1<<1)) != 0)) byte |= 1 << 2;
            if (((PORTB & (1<<13)) != 0)) byte |= 1 << 3;
